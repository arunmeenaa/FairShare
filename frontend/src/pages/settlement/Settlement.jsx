...
const StatCard = ({ label, value, subtitle, iconBg, iconColor, textColor, icon }) => (
  <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700">
    ...
    <p className={`mt-1 text-2xl font-bold ${textColor || "text-slate-900"} dark:text-white`}>
      {value}
    </p>
    {subtitle && <p className="mt-1 text-xs font-semibold text-slate-400 dark:text-slate-500">{subtitle}</p>}
  </div>
);
      ...
      <div className="min-h-[calc(100vh-64px)] bg-slate-50 px-4 py-10 dark:bg-slate-950">
        <div className="mx-auto flex min-h-[60vh] max-w-lg items-center justify-center">
          <div className="w-full rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <h1 className="mt-6 text-2xl font-bold text-slate-900 dark:text-white">Select a household</h1>
      ...
      <div className="min-h-[calc(100vh-64px)] bg-slate-50 px-4 py-6 pb-28 sm:px-6 lg:px-8 dark:bg-slate-950">
        <div className="mx-auto max-w-6xl animate-pulse space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[1, 2, 3, 4].map((item) => (
              <div key={item} className="h-32 rounded-2xl bg-white shadow-sm dark:bg-slate-900" />
            ))}
          </div>
      ...
      <div className="min-h-[calc(100vh-64px)] bg-slate-50 px-4 py-10 dark:bg-slate-950">
        <div className="mx-auto flex min-h-[50vh] max-w-lg items-center justify-center">
          <div className="w-full rounded-3xl border border-red-200 bg-white p-8 text-center shadow-sm dark:border-red-900/60 dark:bg-slate-900">
            ...
            <h2 className="mt-5 text-lg font-bold text-slate-900 dark:text-white">Unable to load settlement</h2>
    ...
    <div className="min-h-[calc(100vh-64px)] bg-slate-50 px-4 py-6 pb-28 sm:px-6 lg:px-8 dark:bg-slate-950">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <p className="text-sm font-medium text-indigo-600 dark:text-indigo-400">{currentHousehold.name}</p>
            <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Settlement</h1>
          ...
          {/* Month / Year Selector */}
          <div className="flex items-center gap-1.5 rounded-2xl border border-slate-200 bg-white p-1.5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <select
              value={month}
              onChange={(e) => setMonth(Number(e.target.value))}
              className="rounded-xl border-0 bg-transparent px-3 py-2 text-sm font-semibold text-slate-700 outline-none transition focus:bg-slate-50 dark:text-slate-200 dark:focus:bg-slate-800"
            >
        ...
        {/* ================= PERIOD BANNER ================= */}
        <div className="flex items-center gap-4 rounded-2xl border border-indigo-100 bg-indigo-50 p-5 dark:border-indigo-900/60 dark:bg-indigo-950/40">
          ...
          <div>
            ...
            <h2 className="mt-0.5 text-lg font-bold text-indigo-950 dark:text-indigo-100">
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="border-b border-slate-100 p-6 dark:border-slate-800">
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
              <div>
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">Who pays whom?</h2>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          ...
          {transactions.length === 0 ? (
            <div className="px-6 py-16 text-center">
              ...
              <h3 className="mt-5 text-lg font-bold text-slate-900 dark:text-white">Everyone is settled</h3>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {transactions.map((transaction) => {
                ...
                return (
                  <div key={transaction._id} className="p-5 transition sm:p-6 hover:bg-slate-50/60 dark:hover:bg-slate-800/40">
                      ...
                      <div className="flex min-w-0 items-center gap-3.5">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="mt-0.5 flex flex-wrap items-center gap-2">
                            ...
                            <span className="font-semibold text-slate-900 dark:text-white">{toName}</span>
                          </div>
